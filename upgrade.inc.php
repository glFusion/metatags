...
{
    global $_CONF, $_TABLES, $_METATAGS_CONF, $_META_DEFAULTS;

    if (isset($_PLUGIN_INFO[$_METATAGS_CONF['pi_name']])) {
        if (is_array($_PLUGIN_INFO[$_METATAGS_CONF['pi_name']])) {
            // glFusion >= 1.6.6
            $current_ver = $_PLUGIN_INFO[$_METATAGS_CONF['pi_name']]['pi_version'];
        } else {
            // legacy
            $current_ver = $_PLUGIN_INFO[$_METATAGS_CONF['pi_name']];
        }
    } else {
        return false;
    }
    ...
    if (!COM_checkVersion($current_ver, '1.0.4')) {
        ...
        $current_ver = '1.0.4';
           ...
           $c->add('replace', $_META_DEFAULTS['replace'], '*text', 0, 0, NULL, 20, TRUE, 'metatags');
           $c->add('show_editor', $_META_DEFAULTS['show_editor'], 'select', 0, 0, 1, 40, TRUE, 'metatags');
    ...
    if (!COM_checkVersion($current_ver, '1.0.7')) {
        ...
        $sql = "UPDATE {$_TABLES['groups']}
                SET grp_gl_core = 2
                WHERE grp_name='{$_METATAGS_CONF['pi_name']} Admin'";
        DB_query($sql,1);
        if (DB_error()) {
    ...
    CTL_clearCache($_METATAGS_CONF['pi_name']);
