...
// | $Id::                                                                   $|
// +--------------------------------------------------------------------------+
// | Based on the MetaTags Plugin for Geeklog CMS                             |
...
require_once $_CONF['path'].'/plugins/metatags/autoinstall.php';

USES_lib_install();

if (!SEC_inGroup('Root')) {
    // Someone is trying to illegally access this page
    COM_errorLog("Someone has tried to illegally access the MetaTags install/uninstall page.  User id: {$_USER['uid']}, Username: {$_USER['username']}, IP: $REMOTE_ADDR",1);
...
if (SEC_checkToken()) {
    ...
    if ($action == 'install') {
    	...
    	if (plugin_uninstall_metatags('installed')) {
    		/**
    		* Redirects to the plugin editor
    		*/
...
// +---------------------------------------------------------------------------+
// | Metatags Plugin for Geeklog - The Ultimate Weblog                         |
// +---------------------------------------------------------------------------+
// | public_html/admin/plugins/metatags/install.php                            |
// +---------------------------------------------------------------------------+
...
require_once '../../../lib-common.php';
require_once $_CONF['path'] . 'plugins/metatags/config.php';
...
$pi_name         = 'metatags';
$pi_display_name = 'Metatags';
$pi_version      = $_METATAGS_CONF['pi_version'];	// Plugin Version
$gl_version      = $_METATAGS_CONF['gl_version'];	// GL Version plugin for
$pi_url          = $_METATAGS_CONF['pi_url'];		// Plugin Homepage

$base_path       = $_CONF['path'] . 'plugins/' . $pi_name . '/';
$GROUPS          = $_METATAGS_CONF['GROUPS'];
$FEATURES        = $_METATAGS_CONF['FEATURES'];
$MAPPINGS        = $_METATAGS_CONF['MAPPINGS'];
...
if (SEC_checkToken()) {
    if (COM_applyFilter($_REQUEST['action']) == 'uninstall') {
